...
$(document).on('click', '.pizza-item', function (e) {
  let data = $(this).data();
  let $form = $('#buy-form');
...
$(document).on('change', selectors.additionsField, function () {
  let field = $(this)[0];
  let $qnty = $(field).parent().find('input[type="number"]');
  if ($(field).is(':checked')) {
...
$(document).on('change', selectors.additionsQntyField, function () {
    ...
    let qnty = parseInt($(this).val(), 10) || 0;
    let singlePrice = parseFloat($(this).parent().find('.price').text()) || 0;
